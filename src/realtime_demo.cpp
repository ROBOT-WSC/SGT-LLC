...
private:
  ...
  // std::vector<pcl::PointCloud<pcl::PointXYZRGB>> histoary_cloud;
  std::vector<pcl::PointCloud<pcl::PointXYZRGB>> histoary_cloud_save;
  ...
  void GPSCallback(const sensor_msgs::NavSatFix::ConstPtr &laserOdometry);
...
sem_graph::sem_graph(ros::NodeHandle *pnh) : pnh_(pnh) {
  ...
  GPS_sub = pnh_->subscribe<sensor_msgs::NavSatFix>(gpsTopic, 100, &sem_graph::GPSCallback, this);
...
void sem_graph::pointcloudCallback(const sensor_msgs::PointCloud2::ConstPtr &pc_msg) {
  ...
  if(cont > 150)
  {
    ...
    {
        // boost::format fmt_pose("%s%d_%d.%s");
        // std::ofstream f_save_pose;
        // f_save_pose.open((fmt_pose % "/home/wsc/range_net/src/RangeNetTrt8/describe/loop_des_" % cont % loop_id % "txt").str(), std::fstream::out);
        ...
        // f_save_pose.close();
        if((cur_pose - histoary_pose[loop_id]).norm() <=13)
        { 
...
void sem_graph::GPSCallback(const sensor_msgs::NavSatFix::ConstPtr &laserOdometry){
  Eigen::Vector3d llh;
  double timer = laserOdometry->header.stamp.toSec();
  llh.x() = laserOdometry->longitude;
  llh.y() = laserOdometry->latitude;
  llh.z() =  laserOdometry->altitude;
  if(flag == 0)
  {
      originllh = llh;
      flag = 1;
  }
  Eigen::Vector3d ecef = llh2ecef(llh);
  Eigen::Vector3d oxyz = llh2ecef(originllh);
  ...
  RobotPose.pose.position.x = enu.x();
  RobotPose.pose.position.y = enu.y();
  RobotPose.pose.position.z = enu.z();
