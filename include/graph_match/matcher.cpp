...
//may Descriptor matching
matcher::matcher(Descriptor Ds1, Descriptor Ds2, int label_size, Eigen::Vector3d thred_local){
    // cout<<"matcher constructed"<<endl;
    noNei = Ds1.noNeighbor;
    //the sum number of the descriptors
    size1 = Ds1.size();
    ...
    for(int i = 0; i<size1; i++ ){
        ...
        for(int j = 0; j<size2; j++){
            ...
            float SumTop = 0;
            float enable_num = 0;
            for(int row = 0; row<(Num-1)/3; row++){
                int first_index= row*3+1;
                int second_index= row*3+2;
                int both_index= row*3+3;
                // float first_distance = sqrt(abs(descriptor1(first_index, 0) - descriptor2(first_index, 0)));
                // float second_distance = sqrt(abs(descriptor1(second_index, 0) - descriptor2(second_index, 0)));
                // float both_distance = sqrt(abs(descriptor1(both_index, 0) - descriptor2(both_index, 0)));
                if(descriptor1(first_index, 0) == 0 || descriptor1(second_index, 0) == 0 || descriptor1(both_index, 0) == 0)
                {
                    continue;
                }
                enable_num = enable_num + 1;
                Eigen::Vector3d local_des1(descriptor1(first_index, 0), descriptor1(second_index, 0), descriptor1(both_index, 0));
                Eigen::Vector3d local_des2(descriptor2(first_index, 0), descriptor2(second_index, 0), descriptor2(both_index, 0));
                float cos_score = local_des1.dot(local_des2)/(local_des1.norm()*local_des2.norm());
                float dis_score = sqrt(abs(descriptor1(both_index, 0) - descriptor2(both_index, 0)));
                // SumTop = SumTop + cos_score;
                if(cos_score <= thred_local(0))
                {

                    continue;

                }
                if(dis_score < thred_local[1])
                {
                    SumTop = SumTop + 1;
                }
            }
...
MatrixXi matcher::getGoodMatcher(float global_thred){
    MatrixXi matcherID(size1, 3);
    MatrixXi goodMatcherId;
    matcherID.setZero();
    int count = 0;
...
float matcher::getGoodscore(float global_thred){
    MatrixXi matcherID(size1, 3);
    MatrixXi goodMatcherId;
    VectorXd pick_num(size2);
    VectorXd pick_score(size2);
    pick_num.setZero();
    pick_score.setZero();
    matcherID.setZero();
    int count = 0;
